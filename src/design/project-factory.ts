...
export function createDesignProject(name = "Nouveau projet", domain: DesignDomain = "architecture"): DesignProject {
  ...
  const project: DesignProject = {
    ...
    name,
    domain,
    description: domain === "web" || domain === "uiux" ? "Concept d’interface assisté par SOPHENIC Design." : domain === "product" ? "Concept produit paramétrique assisté par SOPHENIC Design." : "Projet spatial assisté par SOPHENIC Design.",
    unit: domain === "web" || domain === "uiux" ? "px" : "m",
    stage: "analysis",
    createdAt: now,
    updatedAt: now,
    revision: 1,
