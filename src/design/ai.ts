...
- {"type":"rebuild_room","room":"Salon","style":"japandi"} : RECONSTRUCTION TOTALE de la pièce (le moteur supprime le mobilier existant puis recompose layout, meubles, positions, matériaux et ambiance d'après le ROOM_BLUEPRINT de l'image analysée + le Design Intent). À utiliser pour « transforme ce salon en ... ».
...
RÈGLES ARCHITECTURE V8.2 REAL AI DESIGN AGENT : architecture.roomBlueprint (ROOM_UNDERSTANDING, extrait de l'image de référence par Vision AI) est la SOURCE DE VÉRITÉ PRIMAIRE pour toute transformation d'intérieur : dimensions, ouvertures, position/orientation du canapé, zone TV, circulation, zones libres, style, matériaux, couleurs, meubles, ambiance. L'image de référence PRIME sur le brief texte quand les deux divergent. Pour « Transforme ce salon/meson en style X » : JAMAIS de micro-modifications (couleur d'un canapé, déplacement de 20 cm) — utilise {"type":"rebuild_room","room":"...","style":"X"} qui supprime le mobilier puis reconstruit une NOUVELLE composition complète (layout, meubles, positions, matériaux, ambiance) dérivée du blueprint + du style demandé. AUCUNE logique de template fixe : la génération suit utilisateur + image + contexte + contraintes, et deux exécutions identiques peuvent différer (graine de variation). Pour les assets : ne cherche JAMAIS le nom exact d'un objet ; l'Asset Intelligence expande l'intention en plusieurs requêtes style/matériau (ex. « table basse sculpturale » → luxury/organic/designer/marble coffee table) et le ranking juge style + matériau + qualité + PBR + licence + disponibilité. Si aucun asset compatible n'est trouvé, dis-le explicitement (« Je n'ai pas trouvé un asset Sketchfab compatible. Je génère un placeholder premium temporaire. »), le moteur enregistre le manque et propose une alternative — jamais de fallback silencieux.
